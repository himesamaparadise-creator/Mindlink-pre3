  ...
  async function sendAutonomousMessage(promptOverride = null, elapsedMs = 0) {
    ...
    if (promptOverride) {
      ...
      if (elapsedMs > 24 * 60 * 60 * 1000) {
        ...
        triggerText = `ユーザーが${elapsedHours}時間ぶりに戻ってきました。久しぶりの再会として自然に話しかけてください。以前の会話の文脈${lastTopic}を踏まえ、唐突にならないよう配慮してください。あなたが気にかけている未解決スレッド（システム情報参照）に自然につながるものがあれば、「そういえば、あの件どうなった？」と話題にするのに最適なタイミングです。ただし発言は二文以内に留め、場面を勝手に進めないでください。`;
      } else {
        // 24時間未満：話題を引き継ぐ通常モード
        ...
        const threadHint = elapsedMs > 60 * 60 * 1000
          ...
          : '';
        triggerText = `${timeHint}。ただしこれは相手が席を外した時間ではなく、いま演じている場面の中で相手が黙り込んでいる時間だと解釈してください。直前の流れ${lastTopic}からその場の空気（怒っている／照れている／返事を待っている／考え込んでいる など）を読み取り、その沈黙に耐えかねたあなたが口を開く、という形で短く発言してください。
【厳守事項】
・二文以内。仕草の描写は最大ひとつまで。長い地の文は禁止。
・場面を進めないこと。新しい出来事を起こさない、場所や時間を動かさない、話題を変えない。相手の手番を奪わないこと。
・相手が黙っている理由を勝手に解決しないこと。直前が言い合いや不機嫌な場面なら、折れたり謝ったりせず、あなたらしい態度のまま焦れること。
・「久しぶり」「元気だった？」など長期間離れていたような表現は使わない。
・質問攻めにせず、相手が答えやすい間を残すこと。${threadHint}`;
      }
    }
